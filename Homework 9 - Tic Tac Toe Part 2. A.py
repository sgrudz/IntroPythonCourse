...
def isBoardFull(pos, let): # if board count = 9, that means it is full.    
    for let in pos:
        boardcount = 0 #need to initlaize this here
        if let == "O" or "X":
            boardcount = +1
            return boardcount
        if boardcount == 9:
...
while True:
    for i in range(20): #needs for loop, needs to be bigger than 9 in case someone messes up        
        ...
        if isWinner(board, turn) == True: #here, the board variable outlines a position, and turn is the X or O letter
            ...
            print("\nPlayer " + turn + " won!\n")
            break
        if isBoardFull(board, move) == True:
            print("The board is filled, it is a tie game!")
